...
digraph_unwrap_test() ->
    ...
    ?assertEqual(D, graffeo_ets:unwrap(G)),
...
digraph_delete_test() ->
    G = graffeo_ets:new(),
    ok = graffeo_ets:add_vertex(G, a),
    D = graffeo_ets:unwrap(G),
    ?assertMatch([_ | _], digraph:info(D)),
    ok = graffeo_ets:delete(G),
    ?assertError(_, digraph:info(D)).

%% F-31: {handle_only, Op, Backend} raised on non-handle graph
handle_only_guard_test() ->
    MapG = graffeo:new(),
    ?assertError({handle_only, add_vertex, graffeo_map}, graffeo_ets:add_vertex(MapG, a)),
    ?assertError({handle_only, add_edge, graffeo_map}, graffeo_ets:add_edge(MapG, a, b)),
    ?assertError({handle_only, del_edge, graffeo_map}, graffeo_ets:del_edge(MapG, a, b)),
    ?assertError({handle_only, del_vertex, graffeo_map}, graffeo_ets:del_vertex(MapG, a)),
    ?assertError({handle_only, delete, graffeo_map}, graffeo_ets:delete(MapG)),
    ?assertError({handle_only, unwrap, graffeo_map}, graffeo_ets:unwrap(MapG)).
